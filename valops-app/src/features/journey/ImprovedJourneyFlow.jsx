      ...
      <div className="relative mb-16">
        ...
        <div className="flex justify-between items-start relative">
          {stages.map((stage, index) => (
            <div 
              key={stage.id}
              className={`relative z-10 transition-all duration-300 transform ${(stage.id === 'validation' || stage.id === 'implementation') ? 'opacity-70 hover:opacity-100' : ''}`}
              style={{ 
                width: '16%',
              ...
              onClick={() => {
                // Verificar se o estágio está em desenvolvimento
                if (stage.id === 'validation' || stage.id === 'implementation') {
            ...
            >
              ...
              <div className={`p-4 bg-white border-t-4 ${getColorClass(stage.color, 'border')} rounded-lg shadow-md hover:shadow-lg transition-all duration-300 cursor-pointer h-full relative`}>
                {/* Tarja "Em Breve" para estágios específicos */}
                {(stage.id === 'validation' || stage.id === 'implementation') && (
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-yellow-700">
                <p>Os estágios de <strong>Validação</strong> e <strong>Implementação</strong> estão em desenvolvimento e serão disponibilizados nas próximas atualizações. O sistema está sendo construído de forma incremental.</p>
              </div>
            </div>
          </div>
        </div>
